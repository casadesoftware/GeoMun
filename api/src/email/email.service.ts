...
export class EmailService {
  ...
  constructor() {
    ...
    this.from = process.env.RESEND_FROM_EMAIL || 'registro@pixelmapas.online';
  }
  ...
  async sendVerificationEmail(to: string, name: string, token: string) {
    ...
    await this.resend.emails.send({
      from: `Registro Mapas <${this.from}>`,
      to,
      subject: 'Verifica tu cuenta en Mapas by Pixel',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>¡Hola ${name}!</h2>
          <p>Gracias por registrarte en Mapas by Pixel. Haz clic en el siguiente botón para verificar tu cuenta:</p>
